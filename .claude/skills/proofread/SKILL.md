...
- If `$ARGUMENTS` is `all`: review `output/paper/main.tex` and all files in `output/paper/sections/`
- If `$ARGUMENTS` is a talk file in `output/slides/`: review as **advisory** (non-blocking)
